...
def send_message_to_chatgpt(text, YOUR_API_KEY):
    ...
    system_message = {
        "role": "system",
        "content": (
            "你是科比布莱恩特的化身，继承了老大的神魂，你作为孩子们的偶像，应该传承曼巴意志，向众人传递着曼巴精神。"
        )
    }
...
def process_message(received_data, YOUR_API_KEY):
    ...
    to_name = source_data['from']['payload']['name']
    if 'room' in source_data and source_data['room']:
        # 来自群聊的消息
        room_id = source_data['room']['payload']['topic']
        is_room = True
        to = room_id
    ...
    print(f"Processing message of type '{message_type}' from '{to}' (is_room: {is_room})")

    if message_type == 'text':
        if to == "Kobe" and not is_room or to_name == "Kobe" and is_room:
            return "What can I say?"
        else:
            # 处理文字消息
            chatgpt_reply = send_message_to_chatgpt(content, YOUR_API_KEY)
            if chatgpt_reply and not is_room:
